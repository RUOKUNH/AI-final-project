...
class Avgpool(object):
    ...
    def forward(self,X,Ho,Wo):
        ...
        div = self.stride**2
        self.div = div
        self.shape_out = (B,self.Co,Ho,Wo)
        Out = np.zeros(self.shape_out)
        for i in range(Ho):
            for j in range(Wo):
                Out[:,:,i,j] = np.mean(X[:,:,i*self.stride:(i+1)*self.stride,
                                       j*self.stride:(j+1)*self.stride],axis = (-2,-1))
        return Out
    
    def backward(self,delta):
        dx = np.zeros(self.shape_in)
        B,_,Ho,Wo = self.shape_out
        for i in range(Ho):
            for j in range(Wo):
                dx[:,:,i*self.stride:(i+1)*self.stride,j*self.stride:(j+1)*self.stride] \
                    = delta[:,:,i,j,None,None].copy()
        dx /= self.div
        return dx
...
class Fc(object):
    ...
    def backward(self,delta,lr):
        ...
        dw = np.tensordot(self.Data_in,delta,axes = [(0,),(0,)]) / B
        db = np.sum(delta,axis = 0) / B
        self.timestep += 1
...
class Conv(object):
    ...
    def forward(self,X):
        ...
        Out = np.tensordot(A,self.kernel,axes = [(1,4,5),(1,2,3)])
