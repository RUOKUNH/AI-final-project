...
#ifndef BOARD_SLAVE
    //***********************决定电容充电功率的地方***************************
    //如果没收到裁判系统给的最大功率，就假定其为50，否则用从裁判系统读的值
    if (RefereeData.GameRobotState.chassis_power_limit == 0) {
        return (50 + (RefereeData.PowerHeat.chassis_power_buffer - 10)) > 200 ? 200 : (50 +
                                                                                       (RefereeData.PowerHeat.chassis_power_buffer -
                                                                                        10));
    } else {
        return (RefereeData.GameRobotState.chassis_power_limit + (RefereeData.PowerHeat.chassis_power_buffer - 10)) > 200
               ? 200 : (RefereeData.GameRobotState.chassis_power_limit +
                        (RefereeData.PowerHeat.chassis_power_buffer - 10));
    }
    //***********************决定电容充电功率的地方***************************
#else
    return RxAimedPower;
...
void Cap::Handle(void) {
    ...
    FSMHandle();
    ControlHandle();
    if (underVoltage == 0 && GetCapVoltage() < 13.0f) underVoltage = 1;
    else if (underVoltage == 1 && GetCapVoltage() > 17.0f) underVoltage = 0;
    //LEDShowCapVoltage();
}

void Cap::FSMHandle(void) {
    if (Car::car.GetWorkState() == STOP_STATE) SwitchStateTo(CAP_STATE_STOP);
    else {
        switch (capState) {
            case CAP_STATE_STOP:
