...
/**
  ...
  */
float IMU_SPI::inv_sqrt(float x) {
    float halfx = 0.5f * x;
    float y = x;
    long i = *(long *) &y;
...
/**
  ...
  */
uint8_t IMU_SPI::mpu_write_byte(uint8_t const reg, uint8_t const data) {
    MPU_NSS_LOW;
    tx = reg & 0x7F;
...
/**
  ...
  *                 mpu_device_init() function
  */
uint8_t IMU_SPI::mpu_read_byte(uint8_t const reg) {
    MPU_NSS_LOW;
    tx = reg | 0x80;
    HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 55);
    HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 55);
...
/**
  ...
  */
uint8_t IMU_SPI::mpu_read_bytes(uint8_t const regAddr, uint8_t *pData, uint8_t len) {
    MPU_NSS_LOW;
    tx = regAddr | 0x80;
    tx_buff[0] = tx;
    HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 55);
...
/**
  ...
  * @usage  call in ist8310_init() function
	*/
void IMU_SPI::ist_reg_write_by_mpu(uint8_t addr, uint8_t data) {
...
/**
  ...
  * @usage  call in ist8310_init() function
	*/
uint8_t IMU_SPI::ist_reg_read_by_mpu(uint8_t addr) {
    uint8_t retval;
...
* @param    device_address: slave device address, Address[6:0]
	...
	* @note
	*/
void IMU_SPI::mpu_master_i2c_auto_read_config(uint8_t device_address, uint8_t reg_base_addr, uint8_t data_num) {
    /*
...
/**
  ...
  * @usage  call in mpu_get_data() function
	*/
void IMU_SPI::ist8310_get_data(uint8_t *buff) {
...
void IMU_SPI::mpu_get_data() {
    ...
    if (imu.initialTemp == 0) imu.initialTemp = imu.temp;
    /* 2000dps -> rad/s */
    imu.wx = mpu_data.gx / 16.384f / 57.3f;
    imu.wy = mpu_data.gy / 16.384f / 57.3f;
    //store it as float type to avoid accuracy loss and temperature compensation is added
    imu.wz = (mpu_data.gz - mpu_data.gz_offset - (imu.temp - imu.initialTemp) * 0.14f) / 16.384f / 57.3f;
#ifdef IST8310
...
/**
  ...
  * @usage  call in mpu_device_init() function
	*/
uint8_t IMU_SPI::mpu_set_gyro_fsr(uint8_t fsr) {
...
/**
  ...
  * @usage  call in mpu_device_init() function
	*/
uint8_t IMU_SPI::mpu_set_accel_fsr(uint8_t fsr) {
    return mpu_write_byte(MPU6500_ACCEL_CONFIG, fsr << 3);
}

uint8_t IMU::mpu_device_init() {
    imu.FirstEnter = 1;
    MPU_DELAY(100);
...
void IMU::mpu_offset_call(void) {
    ...
    //acceleration in x and z direction is not 0 so these compensations is meaningless
    //主板安装方向改变时要改这个校准
    //哪个轴上加速度应该是0(因为没有roll轴自由度)就保留那个补偿，另外两个置为0
...
#endif
    //set initial value according to the initial value of acceleration
    static float xyPlane, theta, vx, vy, vz, norm;
    xyPlane = sqrt(imu.ax * imu.ax + imu.ay * imu.ay);
    theta = atan2(xyPlane, imu.az);
    vx = imu.ay;
    vy = -imu.ax;
    vz = 0;
    norm = inv_sqrt(vx * vx + vy * vy);
    vx *= norm;
    vy *= norm;
    q0 = cos(theta / 2);
    q1 = sin(theta / 2) * vx;
...
/**
  ...
  * @usage  call in main() function
	*/
void IMU::imu_attitude_update(void) {
    /* yaw    -pi----pi */
    imu.yaw = atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 57.3f;
    /* pitch  -pi/2----pi/2 */
    imu.pit = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3f;
    /* roll   -pi----pi  */
    imu.rol = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.3f;
    imu.negative_wz=-imu.wz;
    imu.negative_wy=-imu.wy;
	imu.negative_yaw=-imu.yaw;
	imu.negative_pit=-imu.pit;

}
const float *IMU::GetpData(DataName_e data){
    switch(data){
        case wx: return &(imu.wx); break;
        case wy: return &(imu.wy); break;
        case wz: return &(imu.wz); break;
        case vx: return &(imu.vx); break;
        case vy: return &(imu.vy); break;
        case vz: return &(imu.vz); break;
        case rol: return &(imu.rol); break;
        case yaw: return &(imu.yaw); break;
        case pit: return &(imu.pit); break;
			
        case negative_wz: { return &(imu.negative_wz); break;}
        case negative_wy: { return &(imu.negative_wy); break;}
        case negative_yaw: { return &(imu.negative_yaw); break;}
        case negative_pit: { return &(imu.negative_pit); break;}
