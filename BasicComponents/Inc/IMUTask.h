...
typedef struct
{
	...
	int16_t gx_offset;
	int16_t gy_offset;
	//it is changed to float type to improve accuracy
	int16_t gz_offset;
} mpu_data_t;
...
typedef struct
{
	...
	float negative_wx; /*!< omiga, +- 2000dps => +-32768  so gx/16.384/57.3 =	rad/s */
	float negative_wy;
	float negative_wz;
...
public:
    ...
    //读取IMU数据接口
    inline const imu_t &GetData() const { return imu; };

    //读取IMU数据所在地址的接口
    const float *GetpData(DataName_e data);
};
